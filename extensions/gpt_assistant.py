...
class GptAssistant(interactions.Extension):
    ...
    async def get_replied_messages(self, 
                                   ...
                                   msgs: list):
        while message._referenced_message_id is not None:
            if channel.id != message._referenced_message_id:
                channel = guild.get_channel(message._referenced_message_id)

            message = message.get_referenced_message()
            autor = await self.get_gpt_author_role(message)
    ...
    async def get_thread_messages(self, client: interactions.Client, 
        ...
        async for m in thread.history(limit=None, before=msgs_thread):
            ...
            if m.type == interactions.MessageType.DEFAULT:
                msgs.insert(0, {"role": autor, "content": m.content.replace(f"<@{client.user.id}>", "Ademir"), "name": nome})
    ...
    async def get_attachment_content(attachments: [interactions.Attachment]):
    ...
    async def on_message_create(self, message_create: interactions.events.MessageCreate):
        ...
        if not isinstance(message.channel, interactions.ThreadChannel) and len(msgs) == 2:
            ...
            if  "choices" in result and len(result["choices"]) > 0:
                ...
                channel = await message.channel.create_thread(name=titulo, auto_archive_duration=interactions.AutoArchiveDuration.ONE_HOUR, message=message)
